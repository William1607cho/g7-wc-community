/**
 * Vitest 테스트 환경 설정 (이 저장소 소유)
 *
 * 그누보드7 코어 `resources/js/tests/setup.ts`(코어 7.0.11, sha256 cf61c9ef…)를 옮겨 온 것이다.
 * 예전 vitest.config.ts 는 코어 루트의 이 파일을 직접 읽었기 때문에, 저장소만 받으면 테스트가
 * 시작조차 하지 못했다. 코어 쪽 파일이 바뀌어도 이 사본은 따라가지 않는다.
 */
...
/**
 * IntersectionObserver 폴리필 — DOM 환경이 IntersectionObserver를 기본 제공하지 않을 때
