...
export const UserInfo: React.FC<UserInfoProps> = ({
  ...
  // 회원 판정: 프로필 식별자(uuid)가 없어도 author.status 가 있으면 회원 레코드가 존재한다.
  // 인기글 API(boards/popular)는 작성자 uuid 를 싣지 않고 status 만 싣는다. 삭제된 계정·비회원 글은
  // status 가 null 이라 비회원으로 남는다. (author.id 는 삭제된 계정에도 남아 있어 판정에 쓰지 않는다)
  const hasMemberRecord = Boolean(actualUserId) || Boolean(author?.status);
  const actualIsGuest = isGuest || author?.is_guest || !hasMemberRecord;
  ...
  const handleClick = useCallback((e: React.MouseEvent) => {
    ...
    // 비회원이거나 userId(uuid)가 없으면 아무 동작 안 함 (uuid 없는 회원은 프로필 경로를 만들 수 없음)
  ...
  // uuid 없는 회원(인기글 등)은 프로필 경로를 만들 수 없어 클릭 동작이 없으므로 hover/커서 표시도 하지 않는다
  const nameButtonClass = (actualUserId && (showDropdown || clickable))
    ? `${defaultColorClass} hover:text-primary-600 dark:hover:text-primary-400 cursor-pointer font-medium`.trim()
