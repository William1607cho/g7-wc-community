...
// 회귀: 인기글 API(boards/popular)의 작성자 객체에는 uuid 가 없고 id·status 만 있다.
// uuid 유무만으로 비회원을 판정하면 모든 회원 글에 비회원 배지가 붙는다. status 로 회원 레코드 존재를 판정한다.
describe('UserInfo 회원 판정 (uuid 없는 작성자 객체)', () => {
  it('uuid 가 없어도 status 가 있으면 회원으로 표시되어야 함 (비회원 배지 없음)', () => {
    render(
      <UserInfo author={{ id: 2, name: 'WilliamCho', status: 'active', is_guest: false }} />
    );
    expect(screen.getByText('WilliamCho')).toBeInTheDocument();
    expect(screen.queryByText('비회원')).not.toBeInTheDocument();
  });

  it('uuid 없는 회원은 클릭해도 드롭다운이 열리지 않아야 함 (프로필 경로 없음)', async () => {
    render(
      <UserInfo author={{ id: 2, name: 'WilliamCho', status: 'active', is_guest: false }} showDropdown />
    );
    await act(async () => {
      fireEvent.click(screen.getByText('WilliamCho'));
    });
    expect(screen.queryByTestId('author-dropdown-menu')).not.toBeInTheDocument();
    expect(screen.getByText('WilliamCho').className).not.toContain('cursor-pointer');
  });

  it('삭제된 계정(id 는 남고 uuid·status·name 이 없음)은 비회원 배지가 유지되어야 함', () => {
    render(<UserInfo author={{ id: 15, name: undefined, status: undefined, is_guest: false }} />);
    expect(screen.getByText('비회원')).toBeInTheDocument();
  });

  it('is_guest=true 작성자는 status 와 무관하게 비회원 배지가 표시되어야 함', () => {
    render(<UserInfo author={{ name: '손님', is_guest: true }} />);
    expect(screen.getByText('비회원')).toBeInTheDocument();
  });

  it('author 없이 개별 props 만 넘기고 userId 가 없으면 기존처럼 비회원이어야 함', () => {
    render(<UserInfo name="익명" />);
    expect(screen.getByText('비회원')).toBeInTheDocument();
  });

  it('uuid 가 있는 회원은 기존처럼 클릭 시 드롭다운이 열려야 함', async () => {
    render(<UserInfo author={{ id: 1, uuid: 'user-uuid-1', name: '홍길동', status: 'active' }} showDropdown />);
    await act(async () => {
