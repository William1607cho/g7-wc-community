...
/**
 * WC Community User Template
 *
 * 그누보드7 템플릿 엔진용 사용자 템플릿 컴포넌트 패키지 (Basic 템플릿 기반, 이커머스 제외)
 */

// Logger 설정 (G7Core 초기화 전에도 동작하도록 폴백 포함)
const logger = ((window as any).G7Core?.createLogger?.('Template:wc-community')) ?? {
    log: (...args: unknown[]) => console.log('[Template:wc-community]', ...args),
    warn: (...args: unknown[]) => console.warn('[Template:wc-community]', ...args),
    error: (...args: unknown[]) => console.error('[Template:wc-community]', ...args),
};
