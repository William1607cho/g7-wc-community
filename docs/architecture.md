...
게시판 모듈은 레이아웃이 전부 `admin` 그룹이고 방문자 화면을 갖지 않습니다.
모듈이 방문자 화면을 소유하면 템플릿마다 다른 디자인을 그 모듈이 전부 알아야 하는데, 공개
API 로만 노출하면 템플릿이 자유롭게 구성할 수 있습니다. 그 대가로 **방문자가 보는
게시판 UI 는 사실상 이 템플릿이 전부**입니다. 원본 Basic 의 상점(이커머스) 화면은 이 템플릿에서 뺐습니다.

그 위에 화면 조직 원칙 둘이 있습니다.

- **단일 베이스.** 화면 전부가 `_user_base` 를 상속합니다. 헤더·푸터·모바일 네비와 토스트·모달
  호스트가 거기 있어, 상속하지 않은 화면에서는 전역 UI 가 통째로 없습니다.
- **조각 중심.** 80개 중 49개가 partial 입니다. 화면이 커서가 아니라, 조각을 여러 화면이 공유하기
  때문입니다 — 주소 폼·탭 구조가 그 예입니다.

**서버 코드가 없습니다** — PHP 0줄 · 모델 0 · 라우트 파일 없음 · 훅 발행/구독 0. 데이터는 전부
모듈·코어의 공개 API 에서 오며, 그래서 이 템플릿만으로는 아무 기능도 동작하지 않습니다.
manifest 가 게시판·페이지 모듈과 주소 검색 플러그인을 **의존으로 선언**하는 이유이고,
...
<!-- @intent START -->
```
routes.json  (경로 → 레이아웃. 모두 정적 경로)
     │
     ▼
layouts/_user_base.json  (헤더 · 푸터 · 모바일 네비 · 토스트/모달 호스트 · 콘텐츠 슬롯)
     │  extends
     ▼
layouts/{auth,board,mypage,page,search,users,errors}/*.json   화면 31
     │  partial 참조
     ▼
layouts/partials/**  조각 49
     │  data_sources → 모듈·코어 공개 API
     │  actions      → 코어 빌트인 핸들러 + 이 템플릿의 전용 핸들러 7
     ▼
...
화면에 조각을 끼워 넣고(홈의 g7-home-widgets 섹션·마케팅 동의 항목), 이 템플릿은
