...
```text
1. 유형: 템플릿 (sirsoft-basic 기반, type=user) — 방문자가 보는 사이트 전체 화면(레이아웃 80개). 게시판 모듈은 방문자 화면을 갖지 않으므로 게시판 UI 는 여기가 소유한다. 이커머스 화면은 없다. 서버 코드 0줄
2. 확장 방식: 훅 0/0 — 확장점은 화면 구조다. 다른 확장이 `layout_extensions` 로 조각을 끼우고, `extensions/{확장}/` 오버라이드로 원본 조각을 대체한다
3. 건드리면 안 되는 것: `extends` 없는 독립 레이아웃에서 toast/modal 사용, 목록 이동의 `mergeQuery` 누락, 글 보기 하단 버튼 줄의 forum-addon 앵커(`justify-end` + `if` 안 `is_guest_post`) 제거
...
<!-- @intent START -->
방문자가 보는 **사이트 전체 화면**을 소유하는 사용자(User) 템플릿입니다. 홈·게시판·
마이페이지·인증·오류 화면(레이아웃 80개)이 여기 있습니다.

**이것이 왜 템플릿에 있는가**가 이 확장을 이해하는 열쇠입니다. 게시판 모듈은
레이아웃이 전부 `admin` 그룹이고 방문자 화면을 갖지 않습니다 — 모듈은 관리자 CRUD 와 공개
API 까지만 소유하고, 그 API 를 소비해 실제로 그리는 것은 이 템플릿입니다. 게시판 디자인을 바꾸는
작업은 게시판 모듈이 아니라 **여기**입니다.
...
2. **화면은 조각으로 나눈다.** 80개 중 49개가 partial 입니다. 조각은 여러 화면이 공유하며,
   화면 하나를 고칠 때는 그 화면 이름의 partials 디렉토리를 함께 엽니다.
3. **다른 확장의 조각을 갈아 끼울 수 있다.** `extensions/{확장}/` 오버라이드가 그 장치이며,
...
페이지 모듈과 주소 검색 플러그인을 의존으로 선언하는 이유입니다.
<!-- @intent END -->
...
모듈·코어의 공개 API 를 호출 → 응답을 컴포넌트에 바인딩.
...
| 레이아웃 | 80개 | [레이아웃 목록](docs/layouts.md#레이아웃-목록) |
| 전용 핸들러 | 7개 | [템플릿 전용 핸들러](docs/handlers.md#템플릿-전용-핸들러) |
...
| 확장점 | 어떻게 쓰는가 |
|---|---|
| 레이아웃 80개 | 다른 확장이 `layout_extensions` 로 자기 조각을 끼워 넣습니다 — 홈의 g7-home-widgets 섹션, 마케팅의 회원가입 동의 항목이 그 예입니다 |
| 제공 컴포넌트 79개 | 조각을 만드는 확장이 이 컴포넌트로 화면을 구성합니다. **여기 없는 컴포넌트를 쓰면 그 조각은 이 템플릿에서 렌더되지 않습니다** |
| 전용 핸들러 7개 | 조각의 액션에서 부를 수 있습니다. 모두 이름만으로 부릅니다(네임스페이스 없음) |
...
- [ ] 의존 모듈(게시판·페이지)의 공개 API 응답 형태가 바뀌면 이 템플릿의 화면이 조용히 빈다 — 그 모듈을 올릴 때 함께 확인
...
| 글 보기 하단 버튼 줄(`types/basic/show.json`)의 모양 앵커 제거 | `flex justify-end px-6 py-4 border-t` 와 `if` 안 `is_guest_post` 유지 | g7-forum-addon 이 이 모양으로 줄을 찾아 포럼 위젯을 끼운다 — 없으면 포럼 위젯이 조용히 빠진다(ERROR 로그 1줄뿐) |
